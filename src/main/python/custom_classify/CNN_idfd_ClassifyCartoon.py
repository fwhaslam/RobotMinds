...
root_path = os.path.expanduser( '~/_Workspace/Datasets/KaggleCartoonReduced/cartoon_classification' )
train_path = Path(root_path + "/TRAIN")
test_path = Path(root_path + "/TEST")


image_count = len(list(Path(root_path).rglob('*.jpg')))
...
train_ds = tf.keras.utils.image_dataset_from_directory(
    train_path,
    # validation_split=0.9,       # only use 10%
    # subset="training",
    seed=123,
...
val_ds = tf.keras.utils.image_dataset_from_directory(
    test_path,
    # validation_split=0.1,       # only use 10%
    # subset="validation",
    seed=123,
    image_size=(img_height, img_width),
    batch_size=batch_size
...
pick_class = 'Red_sunflower'
sunflower_url = "https://storage.googleapis.com/download.tensorflow.org/example_images/592px-Red_sunflower.jpg"
sunflower_path = tf.keras.utils.get_file(pick_class, origin=sunflower_url)
...
print(
    "This {} image most likely belongs to {} with a {:.2f} percent confidence."
        .format(pick_class,class_names[np.argmax(score)], 100 * np.max(score))
)

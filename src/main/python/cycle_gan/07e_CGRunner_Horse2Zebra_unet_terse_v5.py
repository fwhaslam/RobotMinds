...
def create_generator_v5(output_channels):
    ...
    skip2 = x = downsample( 256, 2,2 )(x)      # (bs, 64,64, 256 )
    # tf.print('Shape2=',str(tf.shape(x)))
    skip3 = x = downsample( 512, 2,2 )(x)      # (bs, 32,32, 512 )

    ###############################
    # center layers
    x = downsample( 1024, 2,2 )(x)              # (bs, 16,16, 1024)
    # tf.print('Shape3=',str(tf.shape(x)))
    x = upsample(512, 2,2)(x)                  # (bs, 32,32, 512)
    # tf.print('Shape4=',str(tf.shape(x)))

    ###############################
    # upsample layers

    x = merge_layer( x, skip3 )                 # (bs, 3288877777777777777777777,32, 512+512 )
    # tf.print('Shape5=',str(tf.shape(x)))
    x = upsample(256, 2, 2)(x)                  # (bs, 64,64, 512)
    # tf.print('Shape6=',str(tf.shape(x)))

    x = merge_layer( x, skip2 )                 # (bs, 64,64, 256+256 )
    # tf.print('Shape5=',str(tf.shape(x)))
    x = upsample(192, 2, 2)(x)                  # (bs, 128,128, 192)
    # tf.print('Shape6=',str(tf.shape(x)))

    x = merge_layer( x, skip1 )                 # (bs, 128,128, 192+192 )
    # tf.print('Shape8=',str(tf.shape(x)))
    x = upsample(128, 2, 2)(x)                    # (bs, 256,256, 128)
...
generator_first = create_generator_v5( OUTPUT_CHANNELS )
generator_second = create_generator_v5( OUTPUT_CHANNELS )
...
runner = mgr.cyclegan_runner(
        ...
        epochs=50, checkpoint_root='zebra_unet_terse_ckpt/v5' )

...
def downsample(filters, size, strides=2,padding='same'):
    ...
    # return tf.keras.layers.Conv2D(filters, size, strides=strides, padding=padding, activation='selu')
...
def upsample(filters, size, strides=2, padding='same'):
    ...
    return tf.keras.layers.Conv2DTranspose(filters, size, strides=strides,padding=padding, activation='selu')
    # result = tf.keras.Sequential()
...
def create_generator_v6(output_channels):
    r"""Create generator using a smaller unet style design.  2x3x4
    This is the same as v2 but we are using Selu for some of the layers."""
...
generator_first = create_generator_v6( OUTPUT_CHANNELS )
generator_second = create_generator_v6( OUTPUT_CHANNELS )
...
runner = mgr.cyclegan_runner(
        ...
        epochs=50, checkpoint_root='zebra_unet_terse_ckpt/v6' )

runner.run( base_loss=3 )

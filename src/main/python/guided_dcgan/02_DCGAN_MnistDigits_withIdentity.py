...
checkpoint_dir = './guided_dcgan_ckpt/v02'
...
(ds_train_images, ds_train_labels), (_, _) = tf.keras.datasets.mnist.load_data()

ds_train_images = ds_train_images.reshape(ds_train_images.shape[0], 28, 28, 1).astype('float32')
ds_train_images = (ds_train_images - 127.5) / 127.5  # Normalize the images to [-1, 1]

full_set = (ds_train_images,ds_train_labels)

# Batch and shuffle the data, reduce size if necessary ( eg. take() )
train_dataset = tf.data.Dataset.\
    from_tensor_slices( full_set ).\
    take(BUFFER_SIZE).\
    shuffle(BUFFER_SIZE).\
    batch(BATCH_SIZE)
...
def make_generator_model():
    ...
    # append digit embedding to the 100 weights of noise
    embed = tf.keras.layers.Embedding( 10, 3 )(input1)  # [0-9] digit embeds as 3 dimensional dense key
...
save_image_noise = tf.random.normal([num_examples_to_generate, NOISE_DIM])
save_image_digit = tf.random.uniform( (num_examples_to_generate,1), maxval=10, dtype=tf.int32 )

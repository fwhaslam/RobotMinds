...
from stoplight_dataset import OPTION_NAMES
...
#   Display Stuff
    ...
    def draw_some_images( self, work, notes, results, title ):
        r"""Work is an array of images"""

        tf.print('images.shape=',work.shape)
        tf.print('resuts.shape=',results.shape)
        ...
        for index in range(9):
            ...
            sub.title.set_text( OPTION_NAMES[ tf.argmax( results[index] ) ] )
            plt.axis('off')
            plt.imshow( work[index] )
            tf.print('shape=',work[index].shape)
            tf.print('notes=',np.array(notes[index]))
    ...
    def display_results( self, test_images, test_image_set ):
        ...
        self.draw_some_images( images, notes, result, 'originals' )

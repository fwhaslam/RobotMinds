#
#   Source code from:
#       local source 56_NeuralMachineTranslationWithAttention.py
#
#   This code is identical to what is presented on the tutorial page except for:
#       Added load + store to speed up start time
#       displaying embedding weights to investigation
#
#   Code tested with:
...
weights = reloaded.encoder.embedding.variables[0]

print("\nreloaded embedding weights")
print('weights=',weights)
print('abs=',tf.math.abs( weights ) )
print('sum=',tf.reduce_sum( tf.math.abs( weights ), axis=-1 ) )
print('root/sum/sqr=',tf.math.sqrt( tf.reduce_sum( tf.math.square( weights ), axis=-1 )))
# print('var=',reloaded.encoder.embedding.variables.weights[0])
# print('wgt=',reloaded.encoder.embeddings.weights)
# print('dir=',dir(reloaded.encoder.embedding.variables))
# print('shape=',tf.shape(reloaded.encoder.embedding))
...
# # reloaded = tf.saved_model.load(CHECKPOINT_NAME)
#
# successful_load = try_load_weights( CHECKPOINT_NAME, train_translator )
#
...
print('reduced=', tf.reduce_sum( tf.math.abs(work), axis=-1))